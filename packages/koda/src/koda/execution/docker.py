...
class DockerSettings(Protocol):
    bash_execution_docker_image: str | None
...
class DockerCommandExecutor:
    ...
    @staticmethod
    async def _resolved_path(path: str) -> Path:
        return Path(await AnyioPath(path).resolve())
    ...
    async def run(
        ...
        *,
        ...
        resolved_sandbox_dir = await self._resolved_path(sandbox_dir)
        resolved_cwd = await self._resolved_path(cwd)
