...
class TextInputDialog:
    ...
    def __init__(  # noqa: PLR0913 - dialog options are clearer as keyword arguments.
        self,
        title: str,
    ...
    def _build_container(self) -> Frame:
        ...
        detail_rows = []
        if self._detail:
            detail_text: StyleAndTextTuples = [("class:dialog.hint", self._detail)]
            detail_rows = [
                Window(
        ...
        body = HSplit([title_row, separator, *detail_rows, input_row])
        box = Box(
            body,
            padding_left=2,
            padding_right=2,
